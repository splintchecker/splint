...
typedef struct s_usentry
{
  /*@only@*/ uentry entry;
  bool istrace;

  /* 
  ** Trace entries have a level and index that identify the original entry. 
  ** Not used for normal entries.
  */
...
struct _usymtab
{
  ...
  bool     mustBreak;
  exitkind exitCode;
  /*@reldef@*/ /*@only@*/ usentry *entries;
  
  /* Optional.  Used for the global environment to speed lookups. */
  /*@null@*/ /*@only@*/ cstringTable htable;   

  /* Guarded references - these references are not null along this path. */
  /*@i32 This seems unnecessary - the trace entries should cover it...? */
  /*@only@*/ guardSet guards;   
...
/*@constant int invalidScope;@*/
# define invalidScope -1  /* invalid scope */
